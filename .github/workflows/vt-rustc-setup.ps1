$ErrorActionPreference = "Stop"

rustup toolchain install nightly-2025-06-19 --component rustc --component rust-std --component cargo
if ($LastExitCode -ne 0) {
    throw "failed to install nightly toolchain"
}

if ($env:VT_RUSTC -ne "") {
    # Download the rustc compiler
    Write-Host "Downloading holiton rustc"
    Invoke-WebRequest $env:VT_RUSTC -OutFile "vt_rustc.zip" -Headers @{
        "Accept"               = "application/octet-stream"
        "Authorization"        = "Bearer $env:VT_RUSTC_AUTHORIZATION"
        "X-GitHub-Api-Version" = "2022-11-28"
    }

    Write-Host "Extracting holiton rustc"
    Expand-Archive -Path "vt_rustc.zip" -DestinationPath "$(Get-Location)/vt_rustc" -Force

    Write-Host "Linking holiton rustc"
    rustup toolchain link vt-rust "$(Get-Location)/vt_rustc"
    if ($LastExitCode -ne 0) {
        throw "failed to link holiton rust version"
    }

    rustup default vt-rust
    if ($LastExitCode -ne 0) {
        throw "failed to default to the holiton rust version"
    }
}
else {
    Write-Host "holiton rustc download URL not set. Using standard rustc version."
    rustup default nightly-2025-06-19
    # No need to setup anything then
}

rustc -V -v
cargo -V -v