            ...
            .build(|| {
                ...
                if let Some(_tab_bar) = ui.tab_bar("main") {
                    ...
                    if let Some(_) = ui.tab_item("Misc") {
                        ...
                        let mut capture_debug_override =
                            app.capture_debug_override.load(Ordering::Relaxed);
                        if ui.checkbox(
                            obfstr!("Capture debug override (show in OBS/recording)"),
                            &mut capture_debug_override,
                        ) {
                            app.capture_debug_override
                                .store(capture_debug_override, Ordering::Relaxed);
                            app.settings_screen_capture_changed
                                .store(true, Ordering::Relaxed);
                        }
                        if ui.is_item_hovered() {
                            ui.tooltip_text(obfstr!(
                                "Debug-only runtime override. Does not change config file."
                            ));
                        }

                        let capture_status = if app.capture_debug_override.load(Ordering::Relaxed) {
                            "Debug override (visible in capture)"
                        } else if settings.hide_overlay_from_screen_capture {
                            "Protected (hidden from capture)"
                        } else {
                            "Visible (from settings)"
                        };
                        ui.text(format!("Current capture mode: {}", capture_status));
