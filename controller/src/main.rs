...
pub struct Application {
    ...
    pub capture_debug_override: AtomicBool,
}
...
impl Application {
    ...
    pub fn pre_update(&mut self, controller: &mut SystemRuntimeController) -> anyhow::Result<()> {
            ...
            .swap(false, Ordering::Relaxed)
        {
            ...
            let allow_capture = self.capture_debug_override.load(Ordering::Relaxed)
                || !settings.hide_overlay_from_screen_capture;
            controller.toggle_screen_capture_visibility(allow_capture);
            log::debug!(
    ...
    pub fn update(&mut self, ui: &imgui::Ui) -> anyhow::Result<()> {
        ...
        } else if ui.is_key_pressed_no_repeat(Key::F10) {
            let new_state = !self.capture_debug_override.load(Ordering::Relaxed);
            self.capture_debug_override.store(new_state, Ordering::Relaxed);
            self.settings_screen_capture_changed
                .store(true, Ordering::Relaxed);
            log::warn!(
                "Screen capture debug override: {}",
                if new_state {
                    "enabled (overlay visible in capture)"
                } else {
                    "disabled (follow settings)"
                }
            );
        }
    ...
    fn render_overlay(&self, ui: &imgui::Ui, unicode_text: &UnicodeTextRenderer) {
        ...
        if settings.holiton_watermark {
            ...
            if self.capture_debug_override.load(Ordering::Relaxed) {
                let text = "CAPTURE DEBUG: ON (F10)";
                ui.set_cursor_pos([
                    ui.window_size()[0] - ui.calc_text_size(text)[0] - 10.0,
                    52.0,
                ]);
...
struct AppArgs {
    ...
    /// Allow overlay to appear in screen capture on startup (debug only).
    #[arg(long)]
...
fn real_main(args: &AppArgs) -> anyhow::Result<()> {
    ...
    let app = Application {
        ...
        capture_debug_override: AtomicBool::new(args.allow_screen_capture),
    };
